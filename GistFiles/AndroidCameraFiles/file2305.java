...
import com.mykey.shared.widget.ClusterMarkerLocation;
...
public class SearchFragment extends SharedListingsFragment<SearchPresenter> implements SearchView {
    ...
    private ImageView ivCluster;

    private class MarkerRenderer extends DefaultClusterRenderer<ClusterMarkerLocation> {

        private TextView tvClusterSize;
        private IconGenerator mClusterIconGenerator;

        public MarkerRenderer(Context context, GoogleMap map, ClusterManager clusterManager) {
            super(context, map, clusterManager);
            tvClusterSize = (TextView) LayoutInflater.from(activity).inflate(R.layout.view_cluster, null);
        ...
        protected void onBeforeClusterRendered(Cluster<ClusterMarkerLocation> cluster, MarkerOptions markerOptions) {
            ...
            tvClusterSize.setText(Integer.toString(cluster.getSize()));
    ...
    private void initMap(@Nullable Bundle savedInstanceState) {
        ...
        mapView.getMapAsync(new OnMapReadyCallback() {
            ...
            public void onMapReady(GoogleMap map) {
                googleMap = map;
                mClusterManager = new ClusterManager<ClusterMarkerLocation>(activity, googleMap);
                mClusterManager.setRenderer(new MarkerRenderer(
                        activity, googleMap, mClusterManager));
                googleMap.setOnCameraIdleListener(mClusterManager);
                googleMap.setOnMarkerClickListener(mClusterManager);
                googleMap.setOnInfoWindowClickListener(mClusterManager);
                mClusterManager.setAlgorithm(new NonHierarchicalDistanceBasedAlgorithm<ClusterMarkerLocation>());
                mClusterManager.setOnClusterItemClickListener(new ClusterManager.OnClusterItemClickListener<ClusterMarkerLocation>() {
                    ...
                    public boolean onClusterItemClick(ClusterMarkerLocation clusterMarkerLocation) {
                        final Listing listing = clusterMarkerLocation.listing;
                        if (listing != null) {
    ...
    private void addMapMarkers(List<Listing> listings) {
        ...
        for (Listing listing : listings) {
            if (listing.price > 0) {
                ...
                mClusterManager.addItem(new ClusterMarkerLocation(listing, markerOptions));
            }
        }
        updateMapZoom();
    }
