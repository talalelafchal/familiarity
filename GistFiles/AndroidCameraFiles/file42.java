...
public class BucketHelper {
	...
	public static BucketEntry[] loadBucketEntries(ContentResolver resolver, int type) {
		if (HAS_MEDIA_PROVIDER_FILES_TABLE) {
			return loadBucketEntriesFromFilesTable(resolver, type);
		} else {
			return loadBucketEntriesFromImagesAndVideoTable(resolver, type);
		}
	}

	private static void updateBucketEntriesFromTable(ContentResolver resolver, Uri tableUri,
			HashMap<Integer, BucketEntry> buckets) {
		Cursor cursor = resolver.query(tableUri, PROJECTION_BUCKET_IN_ONE_TABLE, BUCKET_GROUP_BY_IN_ONE_TABLE, null,
				null);
		if (cursor == null) {
	...
	private static BucketEntry[] loadBucketEntriesFromImagesAndVideoTable(ContentResolver resolver, int type) {
		HashMap<Integer, BucketEntry> buckets = new HashMap<Integer, BucketEntry>(64);
		if ((type & MEDIA_TYPE_IMAGE) != 0) {
			updateBucketEntriesFromTable(resolver, Images.Media.EXTERNAL_CONTENT_URI, buckets);
		}
		if ((type & MEDIA_TYPE_VIDEO) != 0) {
			updateBucketEntriesFromTable(resolver, Video.Media.EXTERNAL_CONTENT_URI, buckets);
		}
	...
	private static BucketEntry[] loadBucketEntriesFromFilesTable(ContentResolver resolver, int type) {
