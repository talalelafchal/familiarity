...
public class OcrResult {
  ...
  private String[] resultStrArray = new String[2];
  private Paint paint;
...
//    String temptext = text.replace("\n"," ");
    ...
    for(int i=0;i<temp.length;i++){
      Log.e("temp","~~~~~~~~~~~~~~~~~~~~temp["+i+"]====="+temp[i]);
      if(temp[i].length() == 12){
        resultStrArray[0]=temp[i];

        flag12 = true;
      }
      if(temp[i].length() == 8){
        resultStrArray[1]=temp[i];
        flag8 = true;
      }
    }
    this.resultFlag = flag12&flag8;
    this.text = text;
  }
