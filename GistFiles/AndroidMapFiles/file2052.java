...
public class WhiteListActivity extends ListActivity {
  Context context = null;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		listview.setOnItemLongClickListener(new AdapterView.OnItemLongClickListener() {
			...
			public boolean onItemLongClick(AdapterView<?> arg0, View view,
					int arg2, long arg3) {
				// TODO Auto-generated method stub
				TextView txtId = (TextView) view.findViewById(R.id.txtWhiteId);
	...
	private void refresh() {
		List<Map> list = da.getAllByMap(1);
		setListAdapter(new WhiteListAdapter(this, R.layout.list_whitelist, list));
		ListView listView = getListView();
		...
		case R.id.mnuAddNumWhite:
			...
			dialogButton.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					da.add(new BlackList(txt.getText().toString(),1));
					dialog.dismiss();
					refresh();
				}
			});
			dialog.show();
			return true;
		case R.id.mnuAddNumContactWhite:
			Intent intent = new Intent(Intent.ACTION_PICK,
					ContactsContract.CommonDataKinds.Phone.CONTENT_URI);
			startActivityForResult(intent, PICK_CONTACT);
			return true;
		case R.id.mnuAddNumLogWhite:
			getCallLog();
			return true;
		default:
	...
	public void getCallLog() {
        ...
        android.content.DialogInterface.OnClickListener listener = new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialogInterface, int item) {
                ...
                da.add(new BlackList(cursorCall.getString(cursorCall.getColumnIndex(android.provider.CallLog.Calls.NUMBER)),1));
				refresh();
                cursorCall.close();
                return;
            }
		...
		case (PICK_CONTACT): {
			if (resultCode == Activity.RESULT_OK) {				
				...
				if (!phoneNumber.equals("")) {
					da.add(new BlackList(PhoneNumberUtils.stripSeparators(phoneNumber),1));
					refresh();
				} // phoneNumber != ""
			}
			break;
