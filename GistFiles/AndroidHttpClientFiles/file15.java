...
public class Main extends SherlockActivity implements OnClickListener {
	...
	public void testPHP(View view) {
		...
		new Something().execute(json);
	}
	
	protected class Something extends AsyncTask<JSONObject, Void, String> {
		...
		protected String doInBackground(JSONObject... jobj) {
			...
			// Tests the submit_user php script
			
			// Create a new HttpClient and Post Header
			HttpClient httpclient = new DefaultHttpClient();
			HttpPost httppost = new HttpPost(Report.SERVER_URL + Report.SUBMIT_USER);
			JSONObject json = new JSONObject();
			String result = "";
			...
			try {
				...
				// Data to store
				json.put("name", "Sugar Daddy");
				json.put("birthyear", "1000");
				...
				Toast.makeText(Main.this, "JSONException", Toast.LENGTH_SHORT).show();
			} catch (IllegalStateException e1) {
				Toast.makeText(Main.this, "IllegalStateException", Toast.LENGTH_SHORT).show();
			} catch (IOException e1) {
				Toast.makeText(Main.this, "IOException", Toast.LENGTH_SHORT).show();
			}
		...
		protected void onPostExecute(String s) {
			if (s != null)
				...
				Toast.makeText(Main.this, "empty string", Toast.LENGTH_LONG).show();
		}
	}
