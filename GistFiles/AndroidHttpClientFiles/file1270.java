/*
　※ 過去の版は Revisions からご閲覧下さい。

	[2016/01/24] オフライン時に全面広告が表示されず、操作ができなくなるバグを修正。
	  オフライン時は全面広告を表示しないことにした。
	  全面広告を表示できたかを stat に反映できるようにした。
	  表示できた場合、stat に 0 が返る。表示できなかった場合、1 が返る。
	  ただし、devcontrol を呼んでから stat を参照する間に少しの待ち時間
	  (await 100 程度) を入れておくこと。
*/
package tv.hsp;
...
public class HspActivity extends NativeActivity {
	...
	private static boolean interstitialFailed;
	...
	private void prepareInterstitial() {
		...
		interstitialAd.setAdListener(new AdListener() {
			...
			public void onAdLoaded() {
				Log.i("HspActivity", "InterstitialAd loaded.");
			}

			@Override
			public void onAdFailedToLoad(int errorCode) {
				Log.i("HspActivity", "InterstitialAd failed to load.");
				interstitialFailed = true;
			}

			@Override
			public void onAdClosed() {
				Log.i("HspActivity", "InterstitialAd closed.");
				prepareInterstitial();
			}
		});
			...
			_activity.runOnUiThread(new Runnable() {
				...
				public void run() {
					while (!interstitialAd.isLoaded()) {
						if (interstitialFailed) break;
					}
					if (!interstitialFailed) {
						Log.i("HspActivity", "interstitialAd is showing.");
						interstitialAd.show();
						nativepoke(0, 0); // Success
					} else {
						Log.i("HspActivity", "interstitialAd is not shown.");
						nativepoke(0, 1); // Failed
					}
				}
			});
		}
	...
	// Reshows (flag: true) or hides (flag: false) the banner ad.
	private void showAdView(boolean flag) {
		if (flag) {
			_activity.runOnUiThread(new Runnable() {
				@Override
				public void run() {
		...
		if (adView != null) {
			...
			_activity.runOnUiThread(new Runnable() {
				...
				public void run() {
					...
					// pixels due to margins/paddings etc(No way to remove it),
					// so padd it to adjust
