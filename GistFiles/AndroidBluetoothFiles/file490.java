...
private void initLocationManager() {
  ...
  // --------------------------------------------------------------------------------------
        ...
        iBeaconManager.getBeaconParsers().add(new BeaconParser().setBeaconLayout("s:0-1=feaa,m:2-2=10,p:3-3:-41,i:4-20v"));
        // Detect the main Eddystone-UID frame:
        iBeaconManager.getBeaconParsers().add(new BeaconParser().setBeaconLayout("s:0-1=feaa,m:2-2=00,p:3-3:-41,i:4-13,i:14-19"));
        // Detect the telemetry Eddystone-TLM frame:
        iBeaconManager.getBeaconParsers().add(new BeaconParser().setBeaconLayout("x,s:0-1=feaa,m:2-2=20,d:3-3,d:4-5,d:6-7,d:8-11,d:12-15"));
        // Detect the telemetry iBeacon frame:
        iBeaconManager.getBeaconParsers().add(new BeaconParser().setBeaconLayout("m:2-3=0215,i:4-19,i:20-21,i:22-23,p:24-24"));
        // Detect the telemetry Altbeacon frame:
...
// --------------------------------------------------------------------------------------
        ...
        // Eddystone
        if (region.getServiceUuid() == 0xfeaa)
        {
                String type = null;
                if (region.getBeaconTypeCode() == 0x00) {
                        // Eddystone-UID
                        type = "Eddystone-TLM";
                        dict.put("uuid", null);
                        dict.put("major", null);
                        ...
                        dict.put("namespaceId", region.getId1());
                        dict.put("instanceId", region.getId2());
                        // Eddystone-TLM
                        if (region.getExtraDataFields().size() > 0) {
                                type = "Eddystone-TLM";
                                long telemetryVersion = region.getExtraDataFields().get(0);
                                long batteryMilliVolts = region.getExtraDataFields().get(1);
                                long pduCount = region.getExtraDataFields().get(3);
                                long unsignedTemp = (region.getExtraDataFields().get(2) >> 8);
                                double temperature = unsignedTemp > 128 ?
                                                     unsignedTemp - 256 :
                                                     unsignedTemp +(region.getExtraDataFields().get(2) & 0xff)/256.0;
                                long uptime = region.getExtraDataFields().get(4);
                                dict.put("temperature", temperature);
                                dict.put("telemetryVersion", telemetryVersion);
                                dict.put("batteryMilliVolts", batteryMilliVolts);
                                dict.put("pduCount", pduCount);
                                dict.put("uptime", uptime);
                ...
                // Eddystone-URL
                else if (region.getBeaconTypeCode() == 0x10) {
                        dict.put("type", "Eddystone-URL");
                        dict.put("uuid", null);
                        dict.put("major", null);
                        dict.put("minor", null);
                        String url = UrlBeaconUrlCompressor.uncompress(region.getId1().toByteArray());
                        dict.put("url", url);
                }
        ...
        dict.put("BluetoothAddress", region.getBluetoothAddress());
        dict.put("BluetoothName", region.getBluetoothName());
        dict.put("manufacturer", region.getManufacturer());
