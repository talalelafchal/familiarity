...
object IdTagList {
  ...
  def tagFrequency ( listIdTag : List[QuestionPerTag]) : List[(String, Int)] = {
    val tagList =listIdTag.map(x=>x.tag)
    frequencyList(tagList)
  }

  private def frequencyList(tagList: List[Seq[String]]): List[(String, Int)] = {
